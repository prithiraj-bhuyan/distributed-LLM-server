...
set -ue  # treat unset variables as error (but we do NOT use `set -e` globally because task1 has its own handler)
...
submit_task3() {
  ...
  local readinessProbeCheck=$(kubectl get deployment llm-server -o json | jq '.spec.template.spec.containers[0].readinessProbe.httpGet.path')
  local startupProbeCheck=$(kubectl get deployment llm-server -o json | jq '.spec.template.spec.containers[0].startupProbe.httpGet.path')
